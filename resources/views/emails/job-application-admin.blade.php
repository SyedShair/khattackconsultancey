...
<h2 style="margin:0 0 6px; font-size:20px; color:#1a1a2e;">📄 New Job Application</h2>
    <p style="margin:0 0 24px; color:#6b6880; font-size:14px;">
        @if($application->vacancy)
            Someone applied for <strong>{{ $application->vacancy->title }}</strong>.
        @else
            Someone submitted a general application (no open vacancy at the time).
        @endif
    </p>
            ...
            <td style="padding:20px;">
                <table role="presentation" width="100%" cellpadding="6" cellspacing="0" style="font-size:14px; color:#333;">
                    <tr>
                        ...
                        <td style="font-weight:600;">{{ $application->name }}</td>
                    </tr>
                    <tr>
                        <td style="color:#8a869a;">Email</td>
                        <td><a href="mailto:{{ $application->email }}" style="color:#6E1299; text-decoration:none;">{{ $application->email }}</a></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td style="color:#8a869a;">Position</td>
                        <td>{{ $application->vacancy->title ?? 'General Application' }}</td>
                    </tr>
                    @if($application->cover_letter)
                        <tr>
                            <td style="color:#8a869a; vertical-align:top;">Cover Letter</td>
                            <td>{{ \Illuminate\Support\Str::limit($application->cover_letter, 300) }}</td>
                        </tr>
                    @endif
                </table>
            </td>
